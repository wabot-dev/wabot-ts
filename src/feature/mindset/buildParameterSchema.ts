...
import { IMindsetParameterSchema } from './IMindsetToolParameter'
...
): IMindsetParameterSchema {
  const schema: IMindsetParameterSchema = { type: 'string' }
...
export function buildModelSchema(
  ...
  deps: IBuildSchemaDeps,
): { properties: Record<string, IMindsetParameterSchema>; required: string[] } {
  const properties: Record<string, IMindsetParameterSchema> = {}
