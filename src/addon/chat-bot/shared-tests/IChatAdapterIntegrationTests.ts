...
export function runIChatAdapterIntegrationTests(config: IChatAdapterIntegrationTestConfig) {
  ...
  test('Integration: should handle basic text response', async () => {
    ...
    const result = await getAdapter().nextItems(request)

    // Verify response structure
    assert.ok(typeof result === 'object', 'Should return an object')
    assert.ok('nextItems' in result, 'Should have nextItems property')
    assert.ok('usage' in result, 'Should have usage property')

    // Verify nextItems structure
    assert.ok(Array.isArray(result.nextItems), 'nextItems should be an array')
    assert.ok(result.nextItems.length > 0, 'nextItems should not be empty')
    assert.strictEqual(result.nextItems[0]!.type, 'botMessage', 'Should return botMessage type')

    if (result.nextItems[0]!.type === 'botMessage') {
      assert.ok(result.nextItems[0]!.botMessage.text, 'Should have text content')
      assert.ok(result.nextItems[0]!.botMessage.text.length > 0, 'Text should not be empty')
    }
  ...
  test('Integration: should respect system prompt', async () => {
    ...
    assert.ok(result.nextItems.length > 0, 'Should have at least one response item')
    assert.strictEqual(result.nextItems[0]!.type, 'botMessage')
    if (result.nextItems[0]!.type === 'botMessage' && result.nextItems[0]!.botMessage.text) {
      const responseText = result.nextItems[0]!.botMessage.text.toLowerCase()
      ...
      assert.ok(
        hasPirateLanguage,
        `Response should contain pirate language: ${result.nextItems[0]!.botMessage.text}`,
      )
    }
  })
  ...
  test('Integration: should handle tool calling (if supported)', async () => {
    ...
    try {
      ...
      assert.ok(result.nextItems.length > 0, 'Should have at least one response item')
      assert.ok(result.nextItems[0]!.type === 'functionCall' || result.nextItems[0]!.type === 'botMessage')

      if (result.nextItems[0]!.type === 'functionCall') {
        assert.ok(result.nextItems[0]!.functionCall.id, 'Function call should have an id')
        assert.ok(result.nextItems[0]!.functionCall.name, 'Function call should have a name')
        assert.ok(
          ['getCurrentTime', 'calculate'].includes(result.nextItems[0]!.functionCall.name),
  ...
  test('Integration: should maintain conversation context', async () => {
    ...
    if (result.nextItems[0]!.type === 'botMessage' && result.nextItems[0]!.botMessage.text) {
      ...
      assert.ok(
        ...
        `Response should mention Alice: ${result.nextItems[0]!.botMessage.text}`,
      )
    }
  })
  ...
  test('Integration: should handle invalid requests appropriately', async () => {
    ...
    await assert.rejects(() => getAdapter().nextItems(request), {
      message: /empty/i,
    })
  })
  ...
  test('Integration: should handle very long input', async () => {
    ...
    assert.strictEqual(result.nextItems[0]!.type, 'botMessage')
    if (result.nextItems[0]!.type === 'botMessage' && result.nextItems[0]!.botMessage.text) {
      assert.ok(result.nextItems[0]!.botMessage.text.length > 0, 'Should handle long input')
    }
